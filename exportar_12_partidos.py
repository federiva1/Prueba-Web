# exportar_12_partidos.py
# Genera un único Excel con los 12 partidos del Apertura 2026.
# Los primeros 2 usan JSONs ya scrapeados; los 10 restantes se scrapean ahora.
...
OUTPUT_FILE = r"C:\Users\feder\OneDrive\Escritorio\AJ_Apertura2026_Completo.xlsx"

PARTIDOS = [
    {"fecha": "2026-01-26", "rival": "Sarmiento",               "local": True,  "resultado": "1 - 0", "url": None,           "json": "next_data.json"},
    {"fecha": "2026-01-30", "rival": "Estudiantes de RC",       "local": False, "resultado": "0 - 0", "url": None,           "json": "nd_estudiantes.json"},
    ...
    {"fecha": "2026-03-23", "rival": "Platense",                "local": True,  "resultado": "1 - 0", "url": "https://www.fotmob.com/matches/argentinos-juniors-vs-club-atletico-platense/3d6gzl#5102044"},
...
def extraer_jugadores(nd):
    ...
    for pid, p in ps.items():
        if str(p.get("teamId", "")) != str(TEAM_ID): continue
        flat = {}
...
def escribir_hoja(wb, partido, players):
    ...
    t.fill      = fill("1A1A2E"); t.font = Font(bold=True, color="FFFFFF", size=12)
    t.alignment = Alignment(horizontal="center", vertical="center")
    for c in range(2, total_cols + 1): ws.cell(1, c).fill = fill("1A1A2E")
...
# Partidos con JSON ya guardado (sin browser)
partidos_locales = [p for p in PARTIDOS if p.get("json")]
partidos_web     = [p for p in PARTIDOS if not p.get("json")]

for partido in partidos_locales:
    with open(partido["json"], encoding="utf-8") as f:
        nd = json.load(f)
    players = extraer_jugadores(nd)
    escribir_hoja(wb, partido, players)
    print(f"[JSON] {partido['fecha']} vs {partido['rival']} — {len(players)} jugadores")

# Partidos que requieren navegación
with sync_playwright() as p:
    ...
    for i, partido in enumerate(partidos_web):
        print(f"[{i+1}/{len(partidos_web)}] {partido['fecha']} vs {partido['rival']} ...", end=" ", flush=True)
        try:
            page.goto(partido["url"], wait_until="domcontentloaded", timeout=45000)
            time.sleep(5)
            nd = page.evaluate("() => window.__NEXT_DATA__")
            if not nd:
                print("ERROR: sin __NEXT_DATA__"); continue
            players = extraer_jugadores(nd)
            escribir_hoja(wb, partido, players)
...
# Reordenar hojas por fecha (ya están en orden pero por las dudas)
wb.save(OUTPUT_FILE)
print(f"\nExcel guardado: {OUTPUT_FILE}")
print(f"Hojas totales: {len(wb.sheetnames)}")
